...
import pytest
from sqlalchemy import Column, Integer, String, create_engine, literal
...
def compare_strings(expected, actual):
    ...
    if expected != actual:
        ...
        for i, (e_char, a_char) in enumerate(zip(expected, actual)):
            if e_char != a_char:
                print(
                    f"Difference at index {i}: expected '{e_char}', got '{a_char}'")
...
def test_substr_in_filter_mssql(session):
    ...
    compiled_query = query.statement.compile(
        dialect=mssql.dialect(), compile_kwargs={
            "literal_binds": True})
...
def test_substr_in_filter_sqlite(session):
    ...
    compiled_query = query.statement.compile(
        dialect=sqlite.dialect(), compile_kwargs={
            "literal_binds": True})
    normalized_sql = normalize_sql(str(compiled_query))
...
def test_substr_in_filter_postgresql(session):
    ...
    compiled_query = query.statement.compile(
        dialect=postgresql.dialect(), compile_kwargs={
            "literal_binds": True})
    normalized_sql = normalize_sql(str(compiled_query))
...
def test_substr_in_filter_mysql(session):
    ...
    compiled_query = query.statement.compile(
        dialect=mysql.dialect(), compile_kwargs={
            "literal_binds": True})
    normalized_sql = normalize_sql(str(compiled_query))
