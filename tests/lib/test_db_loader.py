...
@patch('emptyproject.lib.db_loader.glob.glob')
@patch('emptyproject.lib.db_loader.DB_Loader.load_data')
def test_load_data_from_file(mock_load_data, mock_glob):
    mock_glob.return_value = ['file1.xlsx', 'file2.xlsx']
    
    db_loader = DB_Loader(db_type='sqlite')
    mock_session = MagicMock()
    mock_options = MagicMock()
    
    db_loader.load_data_from_file(mock_session, '*.xlsx', 'table', mock_options)
    
    mock_load_data.assert_any_call(mock_session, 'file1.xlsx', 'table', mock_options)
    mock_load_data.assert_any_call(mock_session, 'file2.xlsx', 'table', mock_options)

@patch('emptyproject.lib.db_loader.log')
@patch('emptyproject.lib.db_loader.project')
@patch('emptyproject.lib.db_loader.CRUDRepository')
def test_load_data(mock_CRUDRepository, mock_project, mock_log, db_loader):
    ...
    mock_log.info.assert_any_call("Loading %s ...", 'test.xlsx')
    mock_xl.load_data.assert_called_once()
    mock_CRUDRepository.create.assert_any_call(mock_session, mock_table_class.return_value)
