...
def test_project_init(mock_safe_load, mock_file, mock_yaml_data):
    ...
    assert project.config_file == os.path.join(base_dir, project_config_filename)
    assert project.config == mock_yaml_data
    mock_file.assert_called_once_with(os.path.join(base_dir, project_config_filename), 'r', encoding='utf-8')
